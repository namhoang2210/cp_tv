          ...
          >
            <Link href={item.link} prefetch={false} >
              <a className="relative block rounded-lg overflow-hidden group">
                <div className='bg-gray-200 w-[150px] h-[240px] rounded-lg'>
                  <ImageFade
                ...
                {item.point && 
                  <CircularProgressbar
                    className={`h-[32px] w-[32px] absolute bottom-[22px] left-2.5 rounded-full border-[2px] ${item.point >= 8 ? "bg-green-600 border-green-600" : item.point >= 5 ? "bg-[#e0a82e] border-[#e0a82e]" : "bg-red-400 border-red-400"}`}
                    value={item.point * 10} 
